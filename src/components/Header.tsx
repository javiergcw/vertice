    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-6 py-5">
        <Link href="/" className="flex items-center" aria-label="Inicio">
          <Image
            src={headerScrolled ? "/VERTICE-03.png" : "/VERTICE-02.png"}
            alt="VÉRTICE"
            width={320}
            height={80}
        ...
        <div className="flex items-center gap-3">
          ...
          <a
            ...
            className={`hidden h-11 w-11 items-center justify-center rounded-lg border transition-colors md:flex ${headerScrolled
          ...
          <button
            type="button"
            className={`flex h-11 w-11 items-center justify-center rounded-lg border md:hidden ${headerScrolled
              ? "border-white/30 text-white hover:bg-white/10"
      ...
      {open && (
        <>
          ...
          <div
            className="fixed left-0 right-0 bottom-0 top-[96px] z-40 bg-black/50 md:hidden"
            onClick={() => setOpen(false)}
          />

          {/* Panel menú */}
          <div
            className={`fixed left-0 right-0 top-[96px] z-50 border-t px-6 py-5 shadow-xl md:hidden ${headerScrolled
              ? "border-white/20 bg-primary"
            ...
            <nav className="flex flex-col gap-4">
              {NAV_LINKS.map(({ label, href }) => {
                ...
                return (
                  <Link
                    key={href}
                    href={href}
                    className={`text-base font-medium ${active
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                <a
                  ...
                  rel="noopener noreferrer"
                  aria-label="LinkedIn de VÉRTICE"
                  className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border transition-colors ${headerScrolled
                    ? "border-white/30 text-white hover:bg-white/10"
                    : "border-border text-muted-foreground hover:border-accent/50 hover:text-accent"
                    }`}
                >
